...
function evaluateArbitrageProfit(exchanges,asset,initbalance,updated){
    try {
        now = Date.now()
        asset = asset.toUpperCase()
        exchanges.forEach(exchange =>{ exchange = exchange.toLowerCase() })
...
function getPrices(exchange,entrycoin,exitcoin,excluded,updated){
    // CALCULATE CONVERSION PRICE FOR 
    let prices = []
    let timeok = false
